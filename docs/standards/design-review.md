...
- **环境守卫 / 特性开关 / dev-only 门控**（后端 service 方法可能有 `IsProduction()` 检查、`NEXT_PUBLIC_APP_ENV` 分支、feature flag 网关等；前端可能有 `import.meta.env.DEV` 条件渲染。一旦 UX 改造要把「dev-only 的调试能力」提升为「正式用户功能」，前后端所有门控都要一起拆掉，否则功能只在一半环境生效）
