...
// ListActiveWithType returns all active assets (is_deleted = 0), optionally
// filtered by asset_type. An empty assetType means "return all types".
// Used by the v2 market overview handler and scheduled analysis jobs.
func (r *AssetRepo) ListActiveWithType(ctx context.Context, assetType string) ([]model.Asset, error) {
	query := `SELECT asset_catalog_id, code, name, name_en, asset_type, exchange, data_source, created_at, updated_at
		FROM rm_asset_catalog
		WHERE is_deleted = 0`
	var args []interface{}
	...
	rows, err := r.pool.Query(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("query active assets with type: %w", err)
	}
	defer rows.Close()
